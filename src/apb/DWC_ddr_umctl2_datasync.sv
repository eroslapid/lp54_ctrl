...
// -------------------------------------------------------------------------
// -- Revision: $Id: //dwh/ddr_iip/umctl5/DWC_ddrctl_lpddr54_MAIN_BR/DWC_ddr_umctl5/src/apb/DWC_ddr_umctl2_datasync.sv#1 $
// -------------------------------------------------------------------------
// Description:
//
   ...
   generate
      if(DETECT_CHANGE) begin: DetectChange
         ...
         always @(posedge s_clk or negedge s_rst_n) begin : s_data_r_PROC
            if (!s_rst_n) begin
               s_data_r <= {DW{1'b0}};
            end else if(s_data_change) begin
               s_data_r <= s_data;
            end
         end
