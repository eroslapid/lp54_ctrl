...
// -------------------------------------------------------------------------
// -- Revision: $Id: //dwh/ddr_iip/umctl5/DWC_ddrctl_lpddr54_MAIN_BR/DWC_ddr_umctl5/src/xpi/DWC_ddr_umctl2_gafifo_split.sv#1 $
// -------------------------------------------------------------------------
// Description:
//            Generic size Asynchronous FIFO with splitted memory array (n words x M bits)
/********************************************************************************
 * Generic size Asynchronous FIFO with splitted memory array (n words x M bits) *
 ********************************************************************************/
`include "DWC_ddrctl_all_defs.svh"
module DWC_ddr_umctl2_gafifo_split (
    wclk,
    wrst_n,
    wr,
    d,
  ...
  localparam WIDTH_UP     =  WIDTH;

  localparam WIDTH_PAR    = WIDTH_UP+PARW;
   
  // BCM07 generates
  // If the depth parameter is an integer power of two (i.e. 4, 8, 16, 32, etc.), 
  // then the FIFO controller reads from RAM addresses 0 through depth -1 requiring a RAM depth of exactly depth.
  // If the depth parameter is an odd value (i.e. 5, 7, 9, 11, etc.), then the RAM depth must be (depth + 1) 
  // to allow addresses that range from 0 to depth. If depth is an even value but not an integer power of two, 
  // then the RAM depth must be (depth+ 2) to allow addresses that range from 0 to depth + 1.

  localparam EXTRA_MEM_DEPTH = (DEPTH==2**ADDR_WIDTH) ? 0 :
                               (DEPTH%2>0) ? 1 : 2;
...
//  wire    [ADDR_WIDTH -1:0]   rd_addr;
  ...
  wire    [COUNT_WIDTH-1:0]   push_wcount_i;
...
// Unused bcm07 pins 
  ...
  wire   [WIDTH_DOWN-1:0]     data_out_unused;
...
`endif // SNPS_ASSERT_ON
  ...
  assign                   pop_fe     = pop_empty;
   ...
   generate
      if (SPLIT==1) begin: ID_split
         ...
         wire    [WIDTH_DOWN-1:0]    d_down;
          ...
          wire [DEPTH+EXTRA_MEM_DEPTH-1:0] valid_i;
          assign d_down  = d[WIDTH_DOWN-1:0];

           DWC_ddr_umctl2_amem_array
            #(
             .DATA_WIDTH      (WIDTH_DOWN),
             .COUNT_WIDTH     (COUNT_WIDTH),
             .DEPTH           (DEPTH+EXTRA_MEM_DEPTH),
             .MEM_MODE        (0),
             .ADDR_WIDTH      (ADDR_WIDTH)) 
           U_DWC_ddrctl_bcm57_id (
            .clk             (wclk),
            .rst_n           (wrst_n),
            .init_n          (1'b1),
            .wr_n            (we_n),
            .data_in         (d_down),
            .wcount          (push_wcount_i),
            .valid           (valid_i),
            .wr_addr         (wr_addr),
            .rd_addr         ('0),
   ...
   assign push_wcount   = push_wcount_i;
...
endmodule //DWC_ddr_umctl2_gafifo_split
